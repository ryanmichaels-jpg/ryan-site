...
<Lede>
I had heard about Strudel for a while and never used it. It is a small language for making music by typing patterns, and I wanted to point it at something odd. Not a normal synth. A modular synth I could draw by hand, the way you would sketch one on a napkin, and then actually play.
</Lede>
...
tldraw is the infinite whiteboard you have probably seen a demo of. The demo is not the product. The product is an SDK: a canvas engine you embed in your own page and extend. You do not hack the whiteboard, you build your own and teach it what a knob is. That difference turns out to be the whole trick, and I will come back to it.

The plan was ordinary. Strudel writes a stream of tiny messages, "play a C2, right now." A cable carries them to a rack of synth modules, and the rack turns each message into sound. The usual rack for this is VCV Rack, a software modular synth: a wall of virtual knobs and patch cables where each module does one dumb job and you wire them into a sound. My only change was to swap VCV out for a rack of modules living on a tldraw canvas.
...
The cable is the interesting part, because MIDI is how two music machines talk. It is the standard invented in the 80s so any keyboard could drive any synth. A MIDI message is tiny, just a few numbers, and it carries the instruction to make a sound and nothing else.

Normally MIDI runs down a physical wire between two boxes. But here both "boxes" are programs on one computer, so there is no wire to plug in. The trick people use is a loopback driver: a pretend cable the operating system creates. One program pushes messages in one end, another hears them come out the other. Two kids in the same room talking through a cardboard tube. The tube adds nothing; it just connects a mouth to an ear. VCV would sit at the ear end, turning each message into voltages that poke its oscillators.

I built the whole thing around that pretend cable. Then I found I did not need it.
...
My canvas modules and Strudel are not two boxes. They are the same JavaScript, running in the same browser tab, sharing one clock. So Strudel does not have to wait for the moment and shout "now." It hands each note over directly, about a second before it is due, with the exact time stamped on it. The message stops being "C2, now" and becomes "C2, to be played at exactly 3.51000 seconds."

That timestamp is the whole difference, because the browser gives you an appointment book for sound. A call like `setValueAtTime(what, when)` books a change for a specific moment, and once it is booked the audio hardware runs it by itself, on its own clock. My code does not even need to be awake when 3.51 arrives.

<Diagram
  src="strudel-timing.html"
  label="Fig. 1"
  caption="The same four gates, sent two ways. A cable fires each one 'now' and the OS smears it; the appointment book lands every one on the sample."
  height={610}
/>
...
<Diagram
  src="strudel-knob-trace.html"
  label="Fig. 2"
  caption="A knob turn, end to end. The drawn circle and the audio node hold the same number; turning the knob just writes it once."
  height={510}
/>
...
The dream was backwards from how software usually gets built. Do not build a UI that looks hand drawn. Hand draw the UI, then make the drawing operational.

So you draw a module. Circles for knobs, small circles for jacks, labels for buttons. Then you select the drawing and bless it. The code scans the selection, finds the circle-ish shapes, and parks an invisible working knob on top of each one, just an arc and a needle that appear when you touch it. Labeled scribbles become buttons. The drawing stays the face, and the machinery hides underneath, like a paper control panel taped over a real one, except the tape is what makes it real.

For the full rack I did this at scale. Each drawn faceplate got a geometry file, the extracted coordinates of every knob, jack, button, and display window in the drawing, and ghost controls were laid onto those coordinates. Then I drew arrows between the jacks. Those arrows are the patch cables, and they actually patch.

<Diagram
  src="drawn-module-pipeline.png"
  label="Fig. 3"
  caption="Two pipelines meet. Your sketch supplies the faceplate; the real module's manual supplies the math that runs underneath it."
/>

## The modules are cloned from real manuals

I did not invent generic modules. I recreated real Eurorack ones, the same modules you would rack up in VCV: Mutable Instruments' Plaits for the macro oscillator, Befaco's Kickall for the kick drum, a clock, an 8-band EQ, MindMeld's MixMaster console, and an audio interface for the final output.
...
Press play and two sound engines light up, both conducted by the same program.
...
<Diagram
  src="one-conductor-two-orchestras.png"
  label="Fig. 4"
  caption="What happens when you press play. One conductor, two orchestras, and a single mixing desk they both run through."
/>

The thing I keep coming back to is how little separates the picture from the sound. A drawn knob is a row in a database, and so is the filter it drives; turning the knob writes one number that both of them read. The rack is not a drawing of a synth sitting next to a synth. It is one object that happens to have a hand-drawn face. That is a strange way to build an instrument, and I am not sure it is the right way. But it is a very direct one, and drawing the thing you are building and then hearing it play is hard to give up once you have done it.

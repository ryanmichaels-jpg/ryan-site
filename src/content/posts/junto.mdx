...
<Diagram
  src="junto-intro.gif"
  label="Fig. 1"
  caption="One recorded session with Junto, cut together. A question comes back with dated quotes, the run posts its own receipt, the org chart draws itself from the rows, and a sentence remixes a play into a new workflow on the engine. Every frame is a real run."
/>
...
Stripe reached the same principle from the other direction. [Their write-up of Kai](https://stripe.dev/blog/meet-stripes-knowledge-ai-platform) says building it required getting three things right: scaling expertise without centralizing it, meeting users wherever they work, and enforcing guardrails that don't exist in code. That first one is this exact sentence. The people who know how to triage an escalation or model a revenue scenario are, in their words, certainly not on the team building the agent infrastructure. The expertise stays where it lives, and the platform carries everything else.
...
I didn't draw that shape unaided. Right as this started, Cerebras [published how they built their internal knowledge base](https://www.cerebras.ai/blog/how-we-built-our-knowledge-base): one shared schema, a connector for every source, threads distilled into notes a question can find. It was answering 15,000 questions a day within three months. I read it before designing anything, and it gave me direction and a useful kind of nerve. [This thread](https://x.com/DBredvick/status/2078150905078206789) restated their design as a numbered list, and point by point it's the shape above. What the list doesn't contain is a single social word. No credit, no remix, no grading. The part that was mine was never the plumbing but everything that makes the plumbing worth sharing.


<Diagram
  src="junto-bredvick-thread.png"
  label="Fig. 5"
  caption="Drew Bredvick restating the Cerebras build as six steps, to 123K views. Step three is the compression the shared memory section runs on: normalize your business into a set of events."
/>
...
So I went looking for an engine instead of writing one. What I found was an open source workflow engine under an Apache license, a YC company named [Sim](https://sim.ai). It had a visual canvas, tables, a knowledge base, scheduled runs, and an API for executing workflows from outside.
...
There turned out to be enough of this to fill an essay of its own, so it got one. The rest of what I learned about making a bot survive real users is in [What I learned creating a Slack bot](/slack-bot/). This post stays on Junto, and the bot lessons live there.
...
[^c4-3]: The research points the same way. [StateFlow](https://arxiv.org/abs/2403.11322) (Wu et al., 2024) found that framing an LLM task as explicit states with allowed transitions raised success rates and cut costs versus letting the model act freely. And [acpx](https://github.com/openclaw/acpx), a protocol client for coding agents, states it as a design principle: routing is constrained choice, never open-ended generation. Anthropic's [engineering write-up on building effective agents](https://www.anthropic.com/engineering/building-effective-agents) names the same pattern: routing classifies an input and hands it to a specialized followup, and workflows buy predictability for well-defined tasks. It's a pattern note rather than an experiment, and the same document recommends the opposite for open-ended work. Mine is the well-defined case.
...
The strongest version of this warning comes from the far end of the road. [Stripe's Kai](https://stripe.dev/blog/meet-stripes-knowledge-ai-platform), their internal knowledge platform, serves most of their company, and their own roadmap still names the problem: users generate a lot of context in their sessions that is currently locked in, and, in their words, that's not how work gets done. Thousands of sessions a day, and the private-chat death is still the one they're engineering against. That's how strong the default is.

...
I built a shared context layer for GTM. Nobody asked me to. I've just watched almost every AI project our team built stop getting used. None of them broke. They quietly stopped mattering, which turned out to be the more interesting problem. The thing I built is called Junto. It lives in Slack, and you build agents on top of it.

- **The problem.** Projects, whether they were Claude projects, dashboards, or platforms we purchased, eventually died. We were all working from different pages. Everyone had their own version of what worked, nobody could see what anyone else had built, and nothing one person figured out made the next person's job easier.
- **The argument.** What all of them were missing was relationships. To the workday, to teammates, to their own failures, and to every other piece of work.
- **The approach.** A layer. The projects were a place you had to go, and a place we had to go competed with our day until it eventually lost. A layer is what other things get built on, and it carries those four relationships itself, so nothing built on top has to remember them.
- **The output.** Two surfaces. Slack as the front door, and a composer where a play is editable blocks on a canvas, built from 1,000+ triggers and integrations. Underneath, an open source engine self-hosted, four typed stores and one flat index. A constitution injected at exactly one seam, a deterministic linter that outranks the model, and the rules that must always hold living below the model where there's nothing to persuade. Every answer comes back with a diagram drawn from the run's own trace, and every play carries who built it, so remixing someone's work pays them.
...
Ramp settled any doubt I had about this direction. In their tooling demo with Deepline, they described their flywheel: expose the central product's data and actions as callable tools, let reps build their own workflows on top, watch what leads to outlier outcomes, and fold the winners back into the core. The slide said "decentralized experiments, centralized gains." The credit layer is my version of watching what wins.
